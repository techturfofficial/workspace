...
router.get('/me', verifyToken, (req, res) => {
  const user = db.prepare('SELECT id,name,email,personal_email,role,secondary_roles,avatar,badge,points,mobile,github_link,linkedin_link,instagram_link,bio,department,branch,site,employment_status,is_active,created_at FROM users WHERE id=?').get(req.user.id);
